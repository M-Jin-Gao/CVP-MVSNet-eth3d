...
def mvsnet_to_gipuma_cam(in_path, out_path, img_size=False):
    ...
    if img_size:
        intrinsic[0] = intrinsic[0] * 896 / img_size[0]
        intrinsic[1] = intrinsic[1] * 480 / img_size[1]
...
def eth_to_gipuma(scan_folder, scan, dtu_test_root, gipuma_point_folder):
    image_folder = os.path.join(dtu_test_root, scan, 'img', )
    print(image_folder)
    cam_folder = os.path.join(dtu_test_root, scan, 'Cameras')
    ...
    for image_name in image_names:
        ...
        img_size = img.size
        img = img.resize((896, 480), resample=Image.NEAREST)
        img.save(out_image_file)

        # shutil.copy(in_image_file, out_image_file)
        mvsnet_to_gipuma_cam(in_cam_file, out_cam_file, img_size)
...
def probability_filter(scan_folder, prob_threshold):
    ...
    image_names = os.listdir(prob_folder)


    for image_name in image_names:
        image_prefix = os.path.splitext(image_name)[0]
        init_depth_map_path = os.path.join(depth_folder, image_prefix + '.pfm')
        prob_map_path = os.path.join(prob_folder, image_prefix+'.pfm')
...
if __name__ == '__main__':
    ...
    for scan in scans:
        ...
        eth_to_gipuma(scan_folder, scan, dtu_test_root, fusibile_workspace)
