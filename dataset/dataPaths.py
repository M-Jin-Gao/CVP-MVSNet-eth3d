...
def getDepth(data_root,mode,scan,view):
    depth_name = str(view).zfill(8)+".pfm"
    if mode == "train":
        scan_path = scan+"/dep/"
    else:
        scan_path = scan+"/dep/"
...
def getImage(data_root,mode,scan,view):

    if mode == "train" or mode == "val":
        scan_path = scan+"/img/"
        image_name = str(view).zfill(8) + ".png"
    else:
        scan_path = scan+"/img/"
        image_name = str(view).zfill(8) + ".jpg"
