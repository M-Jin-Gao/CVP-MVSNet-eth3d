# CVP-MVSNet
# This file modified based code from the following projects.
...
class network(nn.Module):
    ...
    def forward(self, ref_img, src_imgs, ref_in, src_in, ref_ex, src_ex, \
                    depth_min, depth_max,img_path=False):
        ...
        depth_hypos = calSweepingDepthHypo(ref_in_multiscales[:,-1],src_in_multiscales[:,0,-1],ref_ex,src_ex,depth_min, depth_max,self.args.nhypothesis_init)

        ref_volume = ref_feature_pyramid[-1].unsqueeze(2).repeat(1, 1, len(depth_hypos[0]), 1, 1)

        volume_sum = ref_volume
        volume_sq_sum = ref_volume.pow_(2)
        # np_ref_volume = ref_volume.detach().cpu().numpy()
        # np.save('np_ref_volume.npy',np_ref_volume)
        if self.args.mode == "test" or self.args.mode == "val":
            del ref_volume
        for src_idx in range(self.args.nsrc):
            ...
            # np_warped_volume = warped_volume.detach().cpu().numpy()
            # np.save('np_warped_volume'+str(src_idx)+'.npy', np_warped_volume)
            if self.args.mode == "train":
                volume_sum = volume_sum + warped_volume
                volume_sq_sum = volume_sq_sum + warped_volume ** 2
            elif self.args.mode == "test" or self.args.mode == "val":
                volume_sum = volume_sum + warped_volume
                volume_sq_sum = volume_sq_sum + warped_volume ** 2
                del warped_volume
            else: 
        ...
        if self.args.mode == "test" or self.args.mode == "val":
            with torch.no_grad():
                ...
                for i in range(self.args.nscale-2,-1,-1):
                    prob_confidence = nn.functional.interpolate(prob_confidence[None, :], size=None, scale_factor=2,
                                                            mode='bicubic', align_corners=None)[0]
        # g_depth=depth.data.cpu().numpy()[0]
        # plt.figure()
        # plt.axis("off")
        # plt.imshow(g_depth)
        # plt.savefig("depth.png.png")
        # plt.close()
        # plt.colorbar()

        # plt.imsave(g_depth，cmap='gray')
        ...
        for level in range(self.args.nscale-2,-1,-1):
            ...
            # np_cost_volume = cost_volume.detach().cpu().numpy()
            # np.save('cost_volume.npy', np_cost_volume)
            # sys.exit()
